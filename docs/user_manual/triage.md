...
![View Worklows](../assets/triage_view_workflows.png)

### Creating Workflows

You can click on the "Add Workflow" (:material-plus-circle-outline:) button to create a new Workflow object, which will require:
...
You can optionally select the checkbox to apply your Workflow against existing Alerts. To save, click on the :material-check: button.

### Managing Workflows

The page provides two pre-programmed filters:

1. :material-calendar-remove-outline: **Show workflows that were never used**: Workflows that have no record of being to alerts raised by the system.
2. :material-calendar: **Show workflows not used in the past 3 months**: Workflows that have not been used in the last 90 days.

Both queries are useful for performing housecleaning of Workflows that are no longer relevant or need to be adjusted
because they might be raising too often or too less resulting in ineffective alert triage.
