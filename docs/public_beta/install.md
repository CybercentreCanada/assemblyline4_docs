...
You can easily add authentication via your LDAP server by turning on and configuring the LDAP authentication module in the configuration file (`$HOME/assemblyline4_beta_4/config/config.yml`)
...
You can now use an external service provider to authenticate your users by using the oAuth protocol. Simply turn on oAuth authentication by configuring the oAuth authentication module in the configuration file (`$HOME/assemblyline4_beta_4/config/config.yml`)
