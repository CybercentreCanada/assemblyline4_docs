...
##### Installation of Docker on other Linux distributions
...
Every time you run docker-compose commands you must be in the directory were the docker_compose file resides.

    cd $HOME/assemblyline4_beta_3/

Now you can start Assemblyline 4 Beta
...
There are a few things you need to know now that you have an Assemblyline 4 instance started
...
3. You do not have to wait 30 minutes anymore to get the list of signatures, this process only takes a minute now.
...
#### Viewing logs
There is no log-centralization with this deployment, but since everything runs on the same box you can leverage Docker-logging infrastructure.
...
Services are not loaded from your docker_compose file, they are loaded by a component called `scaler`. To view their logs you must use Docker directly.
