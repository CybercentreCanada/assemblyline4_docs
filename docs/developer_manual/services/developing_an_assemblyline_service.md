...
for Assemblyline. It is aimed at individuals with general software development 
knowledge and basic Python skills. In-depth knowledge of the Assemblyline 
framework is not required to develop a service. 
...
This section will guide you through the bare minimum steps required to create a 
running - but functionally useless - service. Each sub-section below outlines the 
steps required for each of the different files required to create an Assemblyline 
service. All files created in the following sub-sections must be placed in a 
common directory, for example `result_sample`. 
...
class ResultSample(ServiceBase):
    ...
    def execute(self, request):
        ...
        #   This service always drops two embedded file which one generates random results and the other empty results.
        ...
        if request.sha256 not in ['d729ecfb2cf40bc4af8038dac609a57f57dbe6515d35357af973677d5e66417a',
            ...
            #     All section scores will be SUM-ed in the service result
            ...
            #     while also providing easy-to-parse data for automated tools.
            ...
            #     The body argument must be a json.dumps() of a dictionary
            json_body = {
                "a_str": "Some string",
                "a_list": ["a", "b", "c"],
                "a_bool": False,
            ...
            # like embedded files, you can add more than one supplementary file
            fd, temp_path = tempfile.mkstemp(dir=self.working_directory)
            with os.fdopen(fd, "w") as myfile:
        ...
        # Randomized results file
            ...
            #   The content of those results does not matter since we've already showed you
            #   all of the different result sections, tagging, heuristics and file-upload functions
...
#### *ServiceBase* class
The main class of your Assemblyline service must inherit from the `ServiceBase` 
class which can be imported from `assemblyline_v4_service.common.base`. 
The `ServiceBase` base class includes many instance variables which can be used 
to access service-related information. The following tables describe all of the 
...
#### *start* function
The `start` function is called when the Assemblyline service is initiated and 
should be used to prepare your service for task execution. This function is 
optional to implement.

#### *get_tool_version* function
The purpose of the `get_tool_version` function is to return a string containing 
all of the tools used by the service. The tool version should be updated to 
reflect changes in the service tools, so that Assemblyline can rescan files on 
the new service version if they are submitted again.

#### *execute* function
The `execute` function is called every time the service receives a new file to 
scan. This is where you should execute your processing code. In the example 
above, we only generate static results for illustration purposes.

The `request` object is provided as an input to the `execute` function. The 
`request` object holds information about the task to be processed by the 
service. The following table describes all of the `request` object variables 
which the service can use. 

| Variable name | Description |
|:---|:---|
| deep_scan | Returns whether the file should be deep scanned or not. Deep scanning usually takes more time and is better suited for files sent manually. |
| file_contents | Returns the raw byte content of the file to be scanned. |
...
The following table describes all of the `request` object functions which the 
service can use.
...
# NOTE: The key names can be anything and the value can be of any type
config:
  str_config: value1
  int_config: 1
  list_config: [1, 2, 3, 4]
...
#                          that the user can change about the service for each of it's scans
...
heuristics:
  - description: This suspicious heuristic fakes masking as a PDF
    filetype: "*"
    heur_id: 1
    name: Masks as PDF
    score: 100
...
update_config:
  ...
  method: run
  # list of source objects from where to fetch files for update and what will be the names of those files on disk
  sources:
...
```
The descriptions of all the valid fields in a `service_manifest.yml` are available [here](service_manifest.md).
...
## Conclusion
After you've completed creating your first service, your `result_sample` directory should have at least the following files:
