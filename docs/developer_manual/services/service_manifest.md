...
| image | Keyword | Yes | Image name always prepended by ${REGISTRY} or ${PRIVATE_REGISTRY} if image not on dockerhub. Append rest of image path. Do not put a / between rest of image path and registry var. Image name always ends in :$SERVICE_TAG|
